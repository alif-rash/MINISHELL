 ...
 *
 * This is a readline library function that replaces the current line being edited
 * with new text and optionally clears the undo information.
 *
 ...
 *
 * This function calculates the total size of environment variables plus the prompt
 * and checks if it exceeds ARG_MAX. If it does, it prints an error message and
 * returns 1 to indicate the argument list is too long.
 *
 * @param shell A pointer to the shell structure containing environment and prompt data.
 ...
 * spaces, operators, brackets, or words. It creates corresponding token objects and appends them
 * to the shell's token list. If an error occurs during token creation, the function returns an error code.
 *
 ...
 *
 * This function starts from shell->index and finds the matching closing bracket
 * for the opening bracket at that position. It handles nested brackets correctly
 * and updates the start and end position pointers.
 *
 * @param str The input string to parse.
 ...
 *
 * @param shell A pointer to the shell structure containing the input prompt and state data.
...
/**
 * @brief Ensures that PWD and OLDPWD environment variables are properly initialized.
 *
 * This function ensures that the PWD and OLDPWD environment variables are properly
 * initialized and updated in the shell's environment list. If PWD is not found,
 * it is created with the current working directory. If OLDPWD is not found,
...
/**
 * @brief Constructs a new environment variable and adds it to the shell's environment list.
 *
 ...
 * 
 * When called with "exit status" as the string parameter, it acts as a getter/setter.
 * If status is not -1, it sets the static exit status. Always returns the current
 * exit status when called with the correct string, or 0 otherwise.
 * 
 * @param str Control string - must be "exit status" to access the static variable
...
/**
 * @brief Handles errors in the shell by setting exit status and printing error messages.
 * 
 * This function handles different types of errors by setting appropriate exit status
 * and calling the correct error printing function based on the error type.
 * 
 * @param shell Pointer to the shell structure (currently unused)
 * @param msg Error message to display
 * @param error_type Type of error (ERROR_GENERIC, ERROR_PERROR, ERROR_SYNTAX, etc.)
 * @param quote_flag Specific error flag that determines the exact error message format
 */
 ...
 * 
 * This function prints error messages with appropriate prefixes (like "minishell: cd: "
 * or "minishell: export: ") based on the error flag, then prints the arguments
 * and error-specific suffixes.
 * 
 ...
 *
 * @param shell Pointer to the shell structure for accessing PWD environment variable
...
/**
 * @brief Implements the export command to set and display environment variables.
 *
...
/**
 * @brief Main expansion function that processes all types of expansions in input.
 *
 * @param input The input string to expand
 * @param shell Pointer to the shell structure
 ...
 * @return Newly allocated string containing the value, or empty string if not found
 */
...
/**
 * @brief Handles signals and sets appropriate exit status based on process termination.
 *
...
/**
 * @brief Handles execution of non-executable files by running them through /bin/sh.
 *
 * @param shell Pointer to the shell structure
