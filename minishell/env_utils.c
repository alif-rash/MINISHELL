...
int assign_value(t_env **env_node, char *env_variable, int *start_index)
{
    ...
    (*env_node)->value = ft_substr(env_variable, *start_index, ft_strlen(env_variable) - *start_index);
    if(!(*env_node)->value)
        return(free_function((void **)&(*env_node)->key, (void **)&(*env_node)->env, NULL, NULL), 1);
    quoted = add_quote((*env_node)->value);
    if(!quoted)
    {
        free((*env_node)->value);
        return(free_function((void **)&(*env_node)->key, (void **)&(*env_node)->env, (void **)&(*env_node), NULL), 1);
    }
    free((*env_node)->value);
...
int parse_key_value(char *env_variable, int flag, t_env **env_node, int *index)
{
    ...
    if(!(*env_node)->key)
        return(free_function((void **)&((*env_node)->env), (void **)env_node, NULL, NULL), 1);
